...
BITRAB_GITIGNORE_CONTENT = """\
# Auto-generated by bitrab — do not edit the ephemeral sections manually.
# Ephemeral runtime directories (job working dirs, logs, artifacts, worktrees)
temp/
logs/
artifacts/
worktrees/
...
# Allow user-managed config and scripts — these SHOULD be committed
# .bitrab-ci.yml
# build_scripts/
"""
...
def ensure_bitrab_dir(project_dir: Path) -> Path:
    """Create ``.bitrab/`` if it does not exist and write a ``.gitignore``.

    The ``.gitignore`` keeps ephemeral runtime output (job dirs, logs,
    artifacts, worktrees) out of source control while explicitly allowing
    user-managed files (``.bitrab-ci.yml``, ``build_scripts/``) to be
    committed.

    Calling this function is idempotent — it is safe to call on every run.
    The ``.gitignore`` is only written when it does not exist yet so that
    manual edits by the user are preserved.
    """
    bd = bitrab_dir(project_dir)
    bd.mkdir(parents=True, exist_ok=True)
    gi = bd / ".gitignore"
    if not gi.exists():
        gi.write_text(BITRAB_GITIGNORE_CONTENT, encoding="utf-8")
    return bd
...
def clean_job_dirs(project_dir: Path) -> int:
    """Delete all job working directories from ``.bitrab/temp/``.
    ...
    """
    temp_path = bitrab_dir(project_dir) / TEMP_DIR
