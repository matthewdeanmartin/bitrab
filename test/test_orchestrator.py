...
def make_executor(tmp_path: Path) -> JobExecutor:
    vm = VariableManager(project_dir=tmp_path)
    return JobExecutor(variable_manager=vm, project_dir=tmp_path)


def make_job(name: str = "my-job", stage: str = "test") -> JobConfig:
    return JobConfig(name=name, stage=stage)


def make_orchestrator(tmp_path: Path, parallelism: int = 1) -> TUIOrchestrator:
...
class TestRunSingleJobQueued:
    def test_puts_sentinel_on_queue(self, tmp_path):
        ...
        run_single_job_queued(job, executor, job_dir, output_queue=q)
    ...
    def test_records_worker_pid(self, tmp_path):
        ...
        run_single_job_queued(job, executor, job_dir, output_queue=q, worker_pids=worker_pids)
    ...
    def test_returns_job_history(self, tmp_path):
        ...
        result = run_single_job_queued(job, executor, job_dir, output_queue=q)
...
class TestRunSingleJobFile:
    def test_creates_log_file(self, tmp_path):
        ...
        run_single_job_file(job, executor, job_dir, log_path=str(log_path))
    ...
    def test_returns_job_history(self, tmp_path):
        ...
        result = run_single_job_file(job, executor, job_dir, log_path=str(log_path))
        assert result is executor.job_history
...
class TestCIFileCallbacks:
    def make_pipeline(self) -> PipelineConfig:
        return PipelineConfig(stages=["build", "test"])

    def make_outcome(self, name: str, success: bool, stage: str = "test") -> JobOutcome:
        return JobOutcome(job=make_job(name=name, stage=stage), success=success)

    def test_on_pipeline_start_prints_stages(self, capsys):
        cb = CIFileCallbacks()
        cb.on_pipeline_start(self.make_pipeline(), max_workers=2)
    ...
    def test_on_stage_start_resets_log_paths(self, tmp_path):
        cb = CIFileCallbacks()
        cb.log_paths = {"stale": tmp_path / "stale.log"}
    ...
    def test_on_stage_complete_prints_failure_marker(self, capsys, tmp_path):
        ...
        cb.on_stage_complete("test", [self.make_outcome("j1", success=False)])
        out = capsys.readouterr().out
        assert "j1" in out
    ...
    def test_on_stage_complete_no_log_file(self, capsys):
        cb = CIFileCallbacks()
        cb.current_stage_jobs = [make_job("j1")]
        cb.log_paths = {}
    ...
    def test_make_output_writer_registers_log_path(self, tmp_path):
        ...
        assert "j1" in cb.log_paths
        assert cb.log_paths["j1"] == job_dir / "output.log"

    def test_get_worker_func_returns_file_worker(self):
        cb = CIFileCallbacks()
        assert cb.get_worker_func() is run_single_job_file
...
class TestTUIOrchestratorControl:
    ...
    def test_reset_clears_worker_pids(self, tmp_path):
        orch = make_orchestrator(tmp_path)
        orch.worker_pids = {"job-a": 12345}
        orch.reset()
        assert orch.worker_pids == {}
