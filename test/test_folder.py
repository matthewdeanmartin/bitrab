...
def _make_file(path: Path, size: int = 16) -> None:
...
# ---------------------------------------------------------------------------
# Unit: _make_run_id / _is_run_id
# ---------------------------------------------------------------------------


def test_make_run_id_format():
    run_id = _make_run_id()
    assert _is_run_id(run_id)


def test_is_run_id_rejects_short():
    assert not _is_run_id("20240101_120000")


def test_is_run_id_rejects_wrong_length():
    assert not _is_run_id("20240101_120000_xxxxxxxx_extra")


def test_is_run_id_rejects_non_hex():
    assert not _is_run_id("20240101_120000_ZZZZZZZZ")
...
def test_scan_folder_counts_job_dirs(tmp_path):
    bd = tmp_path / ".bitrab"
    _make_file(bd / "myjob" / "output.log", size=1024)
    summary = scan_folder(tmp_path)
...
def test_scan_folder_counts_artifacts(tmp_path):
    bd = tmp_path / ".bitrab"
    _make_file(bd / "artifacts" / "build" / "app.zip", size=4096)
    summary = scan_folder(tmp_path)
...
def test_scan_folder_counts_logs(tmp_path):
    ...
    _make_file(log_dir / "summary.txt", size=512)
    summary = scan_folder(tmp_path)
...
def test_scan_folder_size_warning(tmp_path):
    bd = tmp_path / ".bitrab"
    _make_file(bd / "myjob" / "big.bin", size=1024)
...
def test_scan_folder_no_warning_under_threshold(tmp_path):
    bd = tmp_path / ".bitrab"
    _make_file(bd / "myjob" / "small.txt", size=10)
...
def test_clean_artifacts(tmp_path):
    _make_file(tmp_path / ".bitrab" / "artifacts" / "build" / "x.zip", size=256)
...
def test_clean_job_dirs(tmp_path):
    _make_file(tmp_path / ".bitrab" / "job_a" / "output.log", size=128)
    _make_file(tmp_path / ".bitrab" / "job_b" / "output.log", size=64)
...
def test_clean_job_dirs_preserves_artifacts_and_logs(tmp_path):
    _make_file(tmp_path / ".bitrab" / "myjob" / "out.log", size=32)
    _make_file(tmp_path / ".bitrab" / "artifacts" / "build" / "x", size=32)
    write_run_log(
        tmp_path,
        events_json=[],
        summary_text="ok",
...
def test_clean_all(tmp_path):
    _make_file(tmp_path / ".bitrab" / "myjob" / "out.log", size=64)
    _make_file(tmp_path / ".bitrab" / "artifacts" / "x.zip", size=64)
    write_run_log(
        tmp_path,
        events_json=[],
        summary_text="ok",
...
def test_folder_summary_format_exists(tmp_path):
    _make_file(tmp_path / ".bitrab" / "myjob" / "out.log", size=128)
    summary = scan_folder(tmp_path)
    text = summary.format_text()
