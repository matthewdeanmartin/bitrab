...
Reserved for GitLab server-side linting. The command exists, but today it only reports that server-side linting is not
implemented yet.[^cli]
...
and [bitrab/config/capabilities.py](https://github.com/matthewdeanmartin/bitrab/blob/main/bitrab/config/capabilities.py)
